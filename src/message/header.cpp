...
header::header(uint32_t version, hash_digest const& previous_block_hash, hash_digest const& merkle,
...
header& header::operator=(chain::header const& other){
    chain::header::operator=(other);
    return *this;
}
...
bool operator==(header const& x, chain::header const& y) {
    return static_cast<chain::header const&>(x) == y;
}
...
bool operator==(header const& x, header const& y) {
    return static_cast<chain::header const&>(x) == static_cast<chain::header const&>(y);
}

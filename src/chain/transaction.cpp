...
transaction::transaction(uint32_t version, uint32_t locktime, Ins&& inputs, Outs&& outputs)
    : validation{}
    , tx_raw_(version, locktime , std::forward<Ins>(inputs), std::forward<Outs>(outputs))
    , hash_(hash_compute(tx_raw_))
{}

transaction::transaction(transaction_raw const& tx_raw)
    : validation{}
    , tx_raw_(tx_raw)
    , hash_(hash_compute(tx_raw_))
{}

transaction::transaction(transaction_raw&& tx_raw)
    : validation{}
    , tx_raw_(std::move(tx_raw))
    , hash_(hash_compute(tx_raw_))
{}

// transaction::transaction(transaction_raw const& other, hash_digest const& hash)
//     : validation{}
...
transaction::transaction(transaction&& other)
    ...
    , tx_raw_(std::move(other.tx_raw_))
    , hash_(other.hash_)
{
    // // TODO: implement safe private accessor for conditional cache transfer.
    // validation = std::move(other.validation);
...
// explicit
transaction::operator transaction_raw() const {
    return tx_raw_;
}
...
transaction_raw const& transaction::raw() const {
    return tx_raw_;
}
...
// static 
transaction::tx_opt transaction::factory_from_data(data_chunk const& data, bool wire /* = true*/) {
    auto raw = transaction_raw::factory_from_data(data, wire);
    if ( ! raw) return {};
    return transaction(std::move(*raw));
}

// static 
transaction::tx_opt transaction::factory_from_data(std::istream& stream, bool wire /* = true*/) {
    auto raw = transaction_raw::factory_from_data(stream, wire);
    if ( ! raw) return {};
    return transaction(std::move(*raw));
}

// static 
transaction::tx_opt transaction::factory_from_data(reader& source, bool wire /* = true*/) {
    auto raw = transaction_raw::factory_from_data(source, wire);
    if ( ! raw) return {};
    return transaction(std::move(*raw));
}

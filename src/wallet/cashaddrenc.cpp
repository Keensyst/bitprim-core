...
std::string EncodeCashAddr(const libbitcoin::wallet::payment_address& wallet) {
        ...
        if(wallet.version() == 0x00)
            type = PUBKEY_TYPE;
        else type = SCRIPT_TYPE;
        std::vector<uint8_t> data = PackAddrData(wallet.hash(), type);
        return cashaddr::Encode("bitcoincash", data);
}

CashAddrContent DecodeCashAddrContent(const std::string &addr) {
    std::string prefix;
    std::vector<uint8_t> payload;
    std::tie(prefix, payload) = cashaddr::Decode(addr, "bitcoincash");
...
libbitcoin::wallet::payment_address DecodeCashAddrDestination(const CashAddrContent &content) {
    if (content.hash.size() != 20) {
        ...
        libbitcoin::wallet::payment_address ret;
        return ret;
    }

    std::array<uint8_t, 20> arr;
    std::copy_n(content.hash.begin(), 20, arr.begin());
    ...
    return libbitcoin::wallet::payment_address(arr, version);
}
